...
$PublicRepoUrl    = if ($Env:VIPM_PUBLIC_REPO_URL) { $Env:VIPM_PUBLIC_REPO_URL } else { 'https://github.com/elijah286/actor-framework-overview.git' }
...
function Save-PublicVipmPackage($Package, [string] $OutDir) {
    ...
    Invoke-WebRequest -Uri $url -OutFile $outFile -UseBasicParsing -MaximumRedirection 10 -TimeoutSec 300 -Headers @{ 'User-Agent' = 'actor-framework-overview VIPM downloader' } | Out-Null
